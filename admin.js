...
function handleBookForm(e){
  e.preventDefault();
  if(!document.body.classList.contains('admin-mode')){ showToast('Silakan login sebagai admin'); return; }
  const judul = document.getElementById('judul').value.trim();
  const penulis = document.getElementById('penulis').value.trim();
  const deskripsi = document.getElementById('deskripsi').value.trim();
  const file = document.getElementById('bookImage').files[0];
  if(!judul || !penulis){ showToast('Judul & Penulis wajib diisi'); return; }
  if(file){
...
function handleNewsForm(e){
  ...
  if(!judul){ showToast('Judul berita wajib diisi'); return; }
  if(file){
    const r = new FileReader();
